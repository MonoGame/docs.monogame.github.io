...
Start by creating a new `Sprite Effect` in the _SharedContent_ MonoGame Content Builder file located in the `MonoGameLibrary` project, and name it `colorSwapEffect`.
...
[!code-hlsl[](./snippets/snippet-6-07.hlsl?highlight=9-18)]
...
Here is the actual texture with the swapped colors. Download [this image](./images/color-map-1.png) and add it to your `DungeonSlime` Content's "images" folder.  Once downloaded, do not forget to also open the `MGCB editor` in the `DungeonSlime` project and add the "existing item".
...
2. Then replace the `MainPS` shader function to the following, with the highlight showing the progressive effect:
    ...
    [!code-hlsl[](./snippets/snippet-6-12.hlsl?highlight=9-15)]
    ...
    | <video autoplay loop muted style="max-width: 100%; height: auto;" aria-label="Figure 6-13: The color swap effect is working!"><source src="./gifs/color-swap.webm" type="video/webm"></video> |
    | :---------------------------------------------------------------------: |
...
| <video autoplay loop muted style="max-width: 100%; height: auto;" aria-label="Figure 6-15: Colors that are not in the map do not change color"><source src="./gifs/color-swap-2.webm" type="video/webm"></video> |
| :-----------------------------------------------------------------------------------------: |
...
1. Create a new class file in the _MonoGameLibrary/Graphics_ folder called `RedColorMap.cs`:
...
3. Now in the `Draw()` method, we can _optionally_ change the color map based on some condition. In this example, the color map only being set on every other second:
    ...
    [!code-csharp[](./snippets/snippet-6-20.cs?highlight=6-9)]

| <video autoplay loop muted style="max-width: 100%; height: auto;" aria-label="Figure 6-21: The slime's color changes based on time"><source src="./gifs/color-swap-even-seconds.webm" type="video/webm"></video> |
| :-----------------------------------------------------------------------------------------: |
...
1. To do this, replace the `Slime.Draw()` method in the `Slime.cs` class in the _DungeonSlime/GameObjects_ folder to look like this:
...
6. Now, in the GameScene's `Draw()` method, modify the _slime_'s draw invocation to use the new `configureSpriteBatch` callback:
...
| <video autoplay loop muted style="max-width: 100%; height: auto;" aria-label="Figure 6-22: The slime's color changes when it eats"><source src="./gifs/color-swap-slime.webm" type="video/webm"></video> |
| :---------------------------------------------------------------------------------: |
|               **Figure 6-22: The slime's color changes when it eats**               |

> [!NOTE]
> The video above shows using the [dark-purple](./images/color-map-dark-purple.png) color map being used instead of the pink from earlier.
...
| <video autoplay loop muted style="max-width: 100%; height: auto;" aria-label="Figure 6-23: Combining the color swap and saturation effect"><source src="./gifs/color-saturation.webm" type="video/webm"></video> |
| :-----------------------------------------------------------------------------------------: |
...
1. In the `Draw()` method, instead of having an `if` case to start the `SpriteBatch` with different settings, it can always be configured to start with the `_colorSwapMaterial`:
    ...
    [!code-csharp[](./snippets/snippet-6-29.cs?highlight=6-11)]

2. In the `Update()` method, we just need to set the `_saturation` back to `1` if the game is being played:

    [!code-csharp[](./snippets/snippet-6-30.cs?highlight=14-24)]

| <video autoplay loop muted style="max-width: 100%; height: auto;" aria-label="Figure 6-24: The grayscale effect has been restored"><source src="./gifs/grayscale.webm" type="video/webm"></video> |
| :--------------------------------------------------------------------------: |
...
You can find the [complete code sample for this chapter - here](https://github.com/MonoGame/MonoGame.Samples/blob/3.8.5/Tutorials/2dShaders/src/06-Color-Swap-Effect/).
